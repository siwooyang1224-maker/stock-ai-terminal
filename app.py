...
def analyze_stock(ticker):
    try:
        ...
        curr = df.iloc[-1]
        score = 50
        if curr['RSI'] < 30: score += 25
        if curr['Close'] < df['BB_L'].iloc[-1]: score += 15
        if curr['M'] > curr['MS']: score += 10
        if curr['RSI'] > 70: score -= 30
        
        if score >= 75: verdict = "🚀 강력 매수"
        elif score >= 60: verdict = "✅ 매수 검토"
        elif score <= 30: verdict = "🆘 분할 매도"
